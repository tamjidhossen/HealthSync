...
VALID_SYMPTOMS = [
    ...
    "fatigue", "abdominal distention", "regurgitation.1", "symptoms of the kidneys", "melena",
...
def extract_symptoms_from_text(text: str) -> List[str]:
    """
    Extract valid symptoms from user's text message.
    Uses fuzzy matching to identify symptoms mentioned in the text.
    """
    text_lower = text.lower()
    extracted_symptoms = []
    
    # Direct matching for exact symptoms
    for symptom in VALID_SYMPTOMS:
        if symptom.lower() in text_lower:
            extracted_symptoms.append(symptom)
    
    # Additional keyword mapping for common phrases
    symptom_keywords = {
    ...
    for keyword, symptoms in symptom_keywords.items():
...
def format_symptoms_for_ml(symptoms: List[str]) -> str:
    """
    Format extracted symptoms for ML model input (comma-separated string).
    """
